...
If the feature changes backend (Laravel controllers, routes, models): include a task "Add or update Laravel tests for [scope]" with **Files affected** (e.g. `tests/Feature/...php`) and **Acceptance criteria**: `./vendor/bin/sail test`.
...
- [ ] If backend is changed: plan includes a test task with executable criteria (e.g. sail test)
